...
class KNCalc(): 
    def __init__(self,
        ...
        # FIXME
        knlc_dir = '/home/andsantos/phd/research_projects/Main-Injector/python/knlc/'
    ...
    for e in event_list:
        ...
        for j in range(0, len(time_delays)):
            with ProcessPoolExecutor() as executor:
                ...
                results = [executor.submit(do_kncalcs, i, e) for i in range(0, len(second_loop))]  # time_delays
                
                def do_kncalcs(event, i, map_mode=1):
                    ...
                    fracs_at_expt_and_filters = {}
                    fracs_at_expt_and_filters_later = {}
                    fracs_at_expt_and_filters_deep = {}

                    for l, exposure_time in enumerate(exposure_times_calc):
                        for filter in 'griz':
                            fracs_at_expt_and_filters[filter] = get_percentile_at_exp_time(
                                                                    exposure_time,
                                                                    band=filter,
                            ...
                            fracs_at_expt_and_filters_later[filter] = get_percentile_at_exp_time(
                                                                        exposure_time,
                                                                        band=filter,
                                                                        perccentile_dict=percentile_dict_later,
                                                                        teff_type=teff_kind,
                                                                        return_maglin=True
                                                                    )
                            if m_exp:
                                expt_deep = exposure_times_calc_deep[l]
                                fracs_at_expt_and_filters_deep = get_percentile_at_exp_time(
                                                                    expt_deep,
                                                                    band=filter,
                                                                    percentile_dict=percentile_dict_deep,
                                                                    teff_type=teff_kind,
                                                                    return_maglim=True
                                                                )
                        ...
                        maglims_exp[0][l] = round(fracs_at_expt_and_filters['g'][1], 1)
                        maglims_exp[1][l] = round(fracs_at_expt_and_filters['r'][1], 1)
                        maglims_exp[2][l] = round(fracs_at_expt_and_filters['i'][1], 1)
                        maglims_exp[3][l] = round(fracs_at_expt_and_filters['z'][1], 1)
